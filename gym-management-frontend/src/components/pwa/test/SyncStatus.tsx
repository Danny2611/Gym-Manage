...
export const SyncStatus: React.FC<SyncStatusProps> = ({ 
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('sync-update', handleSyncEvent as EventListener);
    };
  }, []);
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {showDetails && (
        <Card className="p-4">
          <h4 className="font-medium text-gray-900 mb-3">Chi tiết đồng bộ</h4>
          <div className="space-y-3">
            {syncItems.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                {(item.status === 'error' || item.status === 'pending' || item.status === 'syncing') && (
                  <Button
