...
import { pushNotificationService, type PushSubscriptionData, type Notification } from '~/services/pwa/pushNotificationService';
...
const subscribeToNotifications = useCallback(async (): Promise<boolean> => {
  ...
  try {
    ...
    if (!p256dhKey || !authKey) {
      console.error('❌ Missing push subscription keys:', { p256dhKey: !!p256dhKey, authKey: !!authKey });
    ...
    const response = await pushNotificationService.subscribe(subscriptionData, deviceInfo);
  ...
  const sendTestNotification = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setState(prev => ({ ...prev, error: response.message ?? 'Unknown error' }));
  ...
  const markAsRead = useCallback(async (notificationIds: string[]) => {
    try {
      const response = await pushNotificationService.markAsRead(notificationIds);
        ...
        setState(prev => ({
          ...
          notifications: prev.notifications.map(notification => 
            notificationIds.includes(notification._id)
              ? { ...notification, status: 'read' as const, read_at: new Date() }
              : notification
          )
        }));
